...
public class RefundResultCallbackOrderConsumer
    ...
    public void onMessage(MessageWrapper<RefundResultCallbackOrderEvent> message) {
        RefundResultCallbackOrderEvent event = message.getMessage();
        RefundCallbackOrderUpdateReqDTO requestParam = new RefundCallbackOrderUpdateReqDTO();
        requestParam.setOrderSn(event.getOrderSn());
        requestParam.setRefundType(event.getRefundTypeEnum() == null ? null : event.getRefundTypeEnum().getType());
        ...
        if (!callbackResult.isSuccess() || !Boolean.TRUE.equals(callbackResult.getData())) {
            throw new ServiceException("退款任务订单回写完成回执失败");
        }
    }
}
