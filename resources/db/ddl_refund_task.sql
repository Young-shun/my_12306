...
(
    ...
    `order_item_record_ids` longtext COMMENT '退款子订单记录ID集合(JSON)',
    `status`              int(3) NOT NULL DEFAULT 0 COMMENT '任务状态: 0-待处理 1-处理中 2-成功 3-失败',
    `error_message`       varchar(500) DEFAULT NULL COMMENT '失败原因',
    `refund_result`       longtext COMMENT '退款结果(第三方返回)',
    `ticket_callback_status` int(3) DEFAULT 0 COMMENT '车票回写完成标记: 0-未完成 1-已完成',
    `order_callback_status`  int(3) DEFAULT 0 COMMENT '订单回写完成标记: 0-未完成 1-已完成',
