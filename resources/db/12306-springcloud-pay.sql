...
(
    ...
    `refund_task_id`  varchar(64)  NOT NULL COMMENT '退款任务ID',
    `order_sn`        varchar(64)  NOT NULL COMMENT '订单号',
    `pay_sn`          varchar(64)  DEFAULT NULL COMMENT '支付单号',
    `status`          int          DEFAULT 0 COMMENT '任务状态：0-待处理，1-处理中，2-成功，3-失败',
    `refund_type`     int          DEFAULT 0 COMMENT '退款类型：0-部分退款，1-全额退款',
    `refund_amount`   int          DEFAULT 0 COMMENT '退款金额（单位：分）',
    `refund_detail`   longtext     DEFAULT NULL COMMENT '退款详情（JSON）',
    `order_item_record_ids` longtext DEFAULT NULL COMMENT '退款子订单记录ID集合（JSON）',
    `retry_count`     int          DEFAULT 0 COMMENT '已重试次数',
    `max_retry_count` int          DEFAULT 3 COMMENT '最大重试次数',
    `next_retry_time` datetime     DEFAULT NULL COMMENT '下次重试时间',
    `error_message`   varchar(500) DEFAULT NULL COMMENT '错误信息',
    `refund_result`   longtext     DEFAULT NULL COMMENT '退款结果（第三方返回）',
    `ticket_callback_status` int   DEFAULT 0 COMMENT '车票回写完成标记：0-未完成 1-已完成',
    `order_callback_status`  int   DEFAULT 0 COMMENT '订单回写完成标记：0-未完成 1-已完成',
    `create_time`     datetime     NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    `update_time`     datetime     NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
    `del_flag`        tinyint(1)   DEFAULT 0 COMMENT '删除标记 0：未删除 1：删除',
    PRIMARY KEY (`id`),
    UNIQUE KEY `uniq_refund_task_id` (`refund_task_id`) USING BTREE,
    KEY `idx_status_create_time` (`status`, `create_time`) USING BTREE,
    KEY `idx_order_sn` (`order_sn`) USING BTREE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='异步退款任务队列表';
